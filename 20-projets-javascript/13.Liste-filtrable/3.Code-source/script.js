...
const seachInput = document.querySelector(".user-list__input")

seachInput.addEventListener("input", filterData)
...
function searchForOccurences(userData, searchedString) {
  ...
  if (searchedWords.length === 2) {
    return (
      (firstName.startsWith(searchedWords[0]) && lastName.startsWith(searchedWords[1])) || (lastName.startsWith(searchedWords[0]) && firstName.startsWith(searchedWords[1]))
    )
  }

  // On peut écrire une potentielle validation (if length > 2) ici.
  return false
}